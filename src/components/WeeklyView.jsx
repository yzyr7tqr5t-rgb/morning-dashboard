...
export default function WeeklyView({ habits, log }) {
  const days = useMemo(() => {
  ...
  return (
    <div className="space-y-6">
      {/* Bar chart */}
      <div>
        <h3 className="text-slate-400 text-xs font-medium uppercase tracking-wider mb-3">7-day completion</h3>
        <div className="flex items-end gap-2 h-24">
          {days.map(({ date, label, pct, completed, total }) => (
            <div key={date} className="flex-1 flex flex-col items-center gap-1">
              <span className="text-slate-500 text-xs">{completed}/{total}</span>
              <div className="w-full bg-slate-800 rounded-t-md relative" style={{ height: '56px' }}>
                <div
                  className={cn(
                    ...
                    pct >= 60 ? 'bg-indigo-600/70' :
                    pct > 0 ? 'bg-slate-600' : 'bg-slate-800'
                  )}
                  style={{ height: `${Math.max(pct, pct > 0 ? 8 : 0)}%` }}
                />
              </div>
              <span className={cn('text-xs', label === 'Today' ? 'text-indigo-300 font-medium' : 'text-slate-500')}>
                {label}
              </span>
            </div>
          ))}
      ...
      {/* History list */}
      <div>
        <h3 className="text-slate-400 text-xs font-medium uppercase tracking-wider mb-3">History</h3>
        <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
          {[...days].reverse().map(({ date, label, completed, total }) => {
            const done = habitMap[date] ?? new Set()
            return (
              <div key={date} className="bg-slate-800/50 border border-slate-700/50 rounded-xl px-4 py-3">
                <div className="flex items-center justify-between mb-2">
                  <span className={cn('text-sm font-medium', label === 'Today' ? 'text-indigo-300' : 'text-slate-300')}>
